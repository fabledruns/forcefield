...
func (f *fakeContext) Agents() []command.AgentSummary {
	if f.agentList != nil {
		return f.agentList
	}
...
func TestNewSession(t *testing.T) {
	ctx := &fakeContext{}
	if err := NewNewSession().Execute(ctx, nil); err != nil {
		t.Fatalf("NewSession.Execute returned an error: %v", err)
	}
	if !ctx.newed {
		t.Fatal("NewSession.Execute did not call ctx.NewSession")
	}
}
